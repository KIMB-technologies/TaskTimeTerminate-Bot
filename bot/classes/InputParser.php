...
class InputParser {
	...
	private const GROUP_NAME_PREG = '/^[A-Za-z0-9]+$/';

	private const TOKEN_VALUE_PREG = self::GROUP_NAME_PREG;

	public static function checkTimeInput(string $t) : bool {
		return !empty($t) && preg_match( self::TIME_INPUT_PREG, $t) === 1;
	}

	public static function checkGroupInput(string $t) : bool {
		return !empty($t) && preg_match( self::GROUP_NAME_PREG, $t) === 1;
	}

	public static function checkTokenInput(string $t) : bool {
		return !empty($t) && preg_match( self::TOKEN_VALUE_PREG, $t) === 1;
	}
